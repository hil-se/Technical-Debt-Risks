...
class techdebt_cnn:
    ...
    def get_data(self):
        d1=pd.read_csv("data/sql12.csv")
        d2=pd.read_csv("data/jruby-1.4.0.csv")
        d3=pd.read_csv("data/apache-jmeter-2.10.csv")
        d4=pd.read_csv("data/apache-ant-1.7.0.csv")
        d5=pd.read_csv("data/apache-jmeter-2.10.csv")
        d6=pd.read_csv("data/argouml.csv")
        d7=pd.read_csv("data/columba-1.4-src.csv")
        d8=pd.read_csv("data/hibernate-distribution-3.3.2.GA.csv")
        d9=pd.read_csv("data/jEdit-4.2.csv")
        ...
        def remove_punct(text):
            ...
            return text_nopunct
        data['clean_comment'] = data['commenttext'].apply(lambda x: remove_punct(x))
    ...
    def preprocess(self):
        ...
        c, l,df= self.get_data()
        #print("\n\n", c[:20], "\n\n\n")
        #print("\n\n",l[:20])
        
        NUM_WORDS=9325 #must be 9325 actually, also do we include \r in filters? 
        ...
        return word_index,tokenizer,X_train, X_val, y_train, y_val, df
    ...
    def get_embeddings(self):
        ...
        word_index,tokenizer, X_train, X_val, y_train, y_val, df=self.preprocess()
        ...
        return embedding_matrix, word_index, tokenizer, X_train, X_val, y_train, y_val, df
    ...
    def build_model(self):
        ...
        embedding_matrix,word_index,tokenizer, X_train, X_val, y_train, y_val, df =self.get_embeddings()
        ...
        sequence_length =9325
        #print(type(y_train))
        ytrue=tf.convert_to_tensor(y_train) #These are my actual labels
        ...
        x = Dense(64, activation='relu')(l_merge) 
        ...
        def get_class_weights(y):
            ...
            classweight =[cw[0][1]]
            classweight.append(cw[1][1])
            return classweight #returns [0.06, 0.94]
            #class_weights = get_class_weights(data.label.values)


        def weighted_loss(ytrue, preds):
            class_weights = get_class_weights(df.label.values)
            normal_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=ytrue , logits=preds))
            weights = tf.reduce_sum(class_weights * ytrue , axis=1)
            weighted_losses = normal_loss * weights       
        ...
        model.save( "corr3_wl_r0d0op0.h5")

        scores = model.evaluate(X_val, y_val, verbose=0)
        print("Model Accuracy: %.2f%%" % (scores[1]*100))

        #model_name="v1-cnn-word2vec"      
        #model = tf.keras.models.load_model('std_256_og1.h5')
        '''scores = model.evaluate(X_val, y_val, verbose=0)
        print("Model Accuracy: %.2f%%" % (scores[1]*100))'''
        ...
        #self.show()
        #import tensorflow as tf
        dtest=pd.read_csv("data/emf-2.4.1.csv")

        def form_label(row):
            if row['classification'] == 'WITHOUT_CLASSIFICATION':
                return 'no'
            else: return 'yes'
        dtest['label'] = dtest.apply (lambda row: form_label(row), axis=1)
        ...
        def remove_punct(text):
            ...
            text_nopunct = re.sub('['+string.punctuation+']', '', text)
            return text_nopunct
        dtest['clean_comment'] = dtest['commenttext'].apply(lambda x: remove_punct(x))
        

        c=dtest[['clean_comment']]
        l=dtest[['label']]
        labels_test=[]
        ...
        for i, row in c.iterrows():
            comments_test.append(row['clean_comment'])
        ...
        Xtest = pad_sequences(Xtest,maxlen=9325)
        ...
        plt.figure()
        plt.plot(model.hist['accuracy'], lw=2.0, color='b', label='train')
        plt.plot(model.hist['val_accuracy'], lw=2.0, color='r', label='val')
        plt.title('CNN label')
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        ...
        plt.figure()
        plt.plot(model.hist['loss'], lw=2.0, color='b', label='train')
        plt.plot(model.hist['val_loss'], lw=2.0, color='r', label='val')
        plt.title('CNN label')
        plt.xlabel('Epochs')
